...
class MyMessageBox(QMessageBox):
    def __init__(self, title = 'Message', msg = None, btn_titles = [], parent = None):
        ...
        # Stylesheet
        self.setStyleSheet("QMessageBox {background-color: qlineargradient( x1:0 y1:0, x2:1 y2:0, stop:0 #9acd32,  stop:1 #E2E2E2); } QLabel {color: #006400; font-size: 20px; font-weight: bold;}")
    ...
    def __create_btns(self):
        btn_style = '''
    QPushButton {
        background: qlineargradient( x1:0 y1:0, x2:1 y2:0, stop:0 #006400,  stop:1 #2980b9);
        padding: 4px 16px;
        font-size: 18px;
        font-weight: bold;
        color: #fff;
    ...
    QPushButton:hover {
        background: qlineargradient( x1:0 y1:0, x2:1 y2:0, stop:0 #F0E68C,  stop:1 #9acd32);
    }
            '''
